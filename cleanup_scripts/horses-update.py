...
try:
    # Get all chunk files
    chunk_files = glob.glob(os.path.join(directory_path, "Horses_chunk_*.csv"))
        ...
        try:
            ...
            if 'Microchip' in df and 'ImportedMicrochip' in df:
                if 'MicrochipType' not in df.columns:
                    ...
                    general_logger.info(f"Created 'MicrochipType' column in {file_path} based on 'ImportedMicrochip'.")
                df.drop(columns=['ImportedMicrochip'], inplace=True)  # Drop the 'ImportedMicrochip' column
                general_logger.info(f"'ImportedMicrochip' column removed in {file_path}.")
            ...
            if 'Name' in df and 'Type' in df:
                ...
                general_logger.info(f"Updated 'Type' for {missing_type_count} rows in {file_path} where it was missing.")
                ...
                general_logger.info(f"Created 'Type' column with default value 2 in {file_path}.")
            ...
            if 'Gender' in df.columns:
                ...
                general_logger.info(f"Updated 'Gender' for {missing_gender_count} rows in {file_path} where it was missing.")
            ...
            if 'IsTattooed' in df.columns:
                ...
                general_logger.info(f"Updated 'IsTattooed' for {missing_tattooed_count} rows in {file_path} where it was NULL.")
            else:
                # Create 'IsTattooed' column if it doesn't exist
                df['IsTattooed'] = 0  # Default value
                general_logger.info(f"Created 'IsTattooed' column with default value 0 in {file_path}.")
