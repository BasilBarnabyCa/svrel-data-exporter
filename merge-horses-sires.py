...
try:
    ...
    sire_chunks = [file for file in os.listdir(chunk_dir) if file.startswith("Sires_chunk_") and file.endswith(".csv")]

    # Step 2: Sort chunks by numerical order
    horse_chunks.sort(key=lambda x: int(x.split("_chunk_")[1].replace(".csv", "")))
    sire_chunks.sort(key=lambda x: int(x.split("_chunk_")[1].replace(".csv", "")))

    # Step 3: Determine the starting index for Sires
    last_horse_chunk_number = int(horse_chunks[-1].split("_chunk_")[1].replace(".csv", "")) if horse_chunks else 0
    starting_sire_chunk_number = last_horse_chunk_number + 1
    ...
    # Step 6: Process Sire chunks and rename them to Horses_chunk_n
    for i, sire_chunk in enumerate(sire_chunks):
        sire_chunk_path = os.path.join(chunk_dir, sire_chunk)
        sire_df = pd.read_csv(sire_chunk_path)

        # Align, clean, and handle IsTattooed
        aligned_sire_df = align_and_clean_columns(sire_df, sample_horse_df)

        # Set mandatory column data for Sires
        aligned_sire_df["Type"] = 0
        aligned_sire_df["MicrochipType"] = 0
        aligned_sire_df["Gender"] = 5

        # Generate a new chunk filename
        new_chunk_number = starting_sire_chunk_number + i
        new_chunk_name = f"Horses_chunk_{new_chunk_number}.csv"
        sire_output_path = os.path.join(chunk_dir, new_chunk_name)

        aligned_sire_df.to_csv(sire_output_path, index=False)
        general_logger.info(f"Processed and saved Sire chunk as: {new_chunk_name}")

        # Move the original Sires chunk to the root output directory
        destination_path = os.path.join(root_output_dir, sire_chunk)
        shutil.move(sire_chunk_path, destination_path)
        general_logger.info(f"Moved original Sire chunk {sire_chunk} to {destination_path}")

    general_logger.info("All Horse and Sire chunks successfully processed, aligned, and Sires chunks moved!")
